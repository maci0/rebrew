...
```bash
rebrew test src/bench/<file>.c          # compile + byte-compare; auto-updates STATUS
rebrew test src/bench/<file>.c --json   # JSON output with byte-level mismatches
rebrew test src/bench/<file>.c --no-promote          # skip STATUS update
rebrew test 0x<VA> --json                  # find by VA (also accepts a symbol name)
rebrew test src/bench/<file>.c --va 0x10001000 \
...
- **PROVEN is sticky** — never demoted by test/verify (set only by `rebrew prove`);
  ...
  run `rebrew test src/bench/<file>.c --force-status` (single-function only)
...
```bash
rebrew split src/bench/multi.c                    # split into individual files
...
```bash
rebrew prove src/bench/<file>.c --json               # prove NEAR_MATCHING → PROVEN
...
rebrew prove src/bench/<file>.c --watch-va 0x10034640 --json  # also compare 4 bytes of memory at this VA (repeatable)

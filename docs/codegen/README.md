...
**13727 records**, one per (toolchain, version, SP, flags, probe,
function) with `{toolchain, version, sp, flags, probe, function,
size, bytes}` — generated from every probe 1-28 object (all 13 MSVC
...
the precomputed **`corpus-matrix.json`** index (487 functions × their

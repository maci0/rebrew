...
| **Byte-exact compare** (`compile.py` → `coff_reloc.py::smart_reloc_compare`) | Console compiles are *more* deterministic than MSVC6 (no incremental link). Matching is the whole goal of N64/GC decomp. |
...
| **Decomp precedents** | Gold standard: SM64, OOT, MM, Perfect Dark, Paper Mario — all **IDO matching decomp** with `decomp.me` `ido-5.3` / `ido-7.1` compilers. `splat` is the intake tool, `asm-processor`/`mips-dis` for matching. Flag space tiny (`-O1`/`-O2`/`-g` + `-non_shared`/`-G0`/`-mgpopt`). |
...
| **PC profile zoo is complete for its era** | 46 profiles (`rebrew toolchain list`): the MSVC line `msvc-1.0` through `msvc-11.0` with its SP variants, `borland-2.0`/`3.1`/`5.5`, `watcom-2.0-win32`/`-win16`, `delphi-1.0`, `gcc-12.3.0`/`14.2.0`, `clang-16.0.4`/`18.1.8`, `mingw-14.2.0`/`16.2.0`, `ido-5.3`/`7.1` — each with a vendored tree + docker image + flag sweep + smoke gate (`borland-2.0` is the one known golden mismatch; see `docs/TOOLCHAIN.md`). | Add console profiles: `psyq` (proxy: `mipsel-elf-gcc` + `maspsx`), `egcs`, `cw_gc` (CodeWarrior GC 1.0–2.7 aliases), `sh-elf-gcc` (`sh2`/`sh4` variants), `shc` (Hitachi), `ee-gcc`, `xbe` (msvc-6.0 alias), `psp-gcc`. Each maps to a `FlagSet`/`Checkbox` list in `flag_data.py` (sync from `decompme/compilers` repo + community flag docs). |
...
| **No legal toolchain vendoring story** | `toolchain/msvc/6.0-win32/` is vendored. | Console SDKs are proprietary (Sony/Nintendo/Sega). Cannot vendor. Pattern: `tools/` is `.gitignore`'d; `rebrew doctor --install-toolchain <profile>` docs the fetch (user-owned SDK dump) and `toolchain_detect` verifies presence. Ship *open* GCC cross toolchains (`mips-elf-gcc`, `powerpc-eabi-gcc`, `sh-elf-gcc`) by default; treat proprietary IDO/CW/PsyQ as optional overlays. |

### 5.4 Object Parsers & Relocation Model (`matcher/parsers.py`, `coff_reloc.py`)
...
| **006** Docker-first toolchains | One **container image per toolchain-version** (Godbolt model) with a wrapper inside; host falls back to vendored `tools/` or PATH binary; `rebrew.toolchain.ToolchainSpec` registry + `rebrew toolchain list/status/pull`. Shared `rebrew.dosbox` for 16-bit. | IDO, CodeWarrior `mwcceppc`, Hitachi `shc`, Psy-Q `ccpsx` are **not** new glue — they are new `ToolchainSpec` entries + `<family>/<version>-<arch>/Dockerfile`s in the rebrew-toolchains checkout. IDO is the IRIX→Linux-static-recomp analogue of DOSBox: prefer community `ghcr.io/decompme/ido-static-recomp` (Linux-native) over inventing an IRIX emulator. This replaces the roadmap's earlier ad-hoc "Docker or Wine" prose with the codified `docker image present → vendored → PATH` preference already tested for `watcom-2.0-win32`/`msvc-1.52`/`delphi-1.0`. |
...
- **Metadata routing** — `STATUS`/`CFLAGS`/`BLOCKER` stay in `rebrew-functions.toml` (never inline). Console `overlay` is metadata too — keep it alongside, not in the marker line.
...
- [ ] **`splat` YAML intake** (`rebrew intake --splat splat.yaml` or auto-detect `splat.yaml`, ADR-002/004 implications): import overlay list as multiple `BinaryInfo` views or as `data_{target}.json` overlays. Store per-overlay `function_structure.json` or `overlay` column. `catalog` + `build_db` become overlay-aware (`db_version 5` migration — see §5.5.2 notes on DB_FORMAT.md/ANNOTATIONS.md: `overlay` field in `rebrew-functions.toml`, not in the marker line; E013 is per-overlay). Pruning key is `(overlay, VA)` per ADR-004.
- [ ] **Compiler profiles** `ido53` / `ido71` / `egcs` (`rebrew/toolchain.py` ToolchainSpec + `ido/<version>-<arch>/Dockerfile` in the rebrew-toolchains checkout, ADR-006): flag axes are tiny — `-O0..2` × `-G 0..128` × `-non_shared`/`-call_shared` × `-mgpopt`/`-mno-gpopt` × `-xgot`/`-mips2` vs `-mips3`. Sync from `decomp.me/platforms/mwcc` and `ehranchov/ido-static-recomp` docs. Provide Docker-based IDO (`ghcr.io/decompme/ido-static-recomp`) + host-native `mips-linux-gnu-gcc` fallback (ToolchainSpec host-fallback seam).
...
- [ ] **E2E proof target**: SM64 or a minimal IPL3+binary homebrew ROM — demonstrate `intake → skeleton → compile with ido-7.1 via Docker → test → verify` for 3 functions. Requires `docs/CODEGEN_PATTERNS_MIPS.md` stub.
...
- [ ] **PS1 profile** `psyq` (`rebrew/toolchain.py`, ADR-006; `mingw-16.2.0`-style posix profile, `mipsel-elf-gcc` command, empty includes/libs): proxy for Psy-Q; add `maspsx` wrapper for ASM-matching stubs (document as `--maspsx` option in `compile.py`). Flag axes: `-O0..2` × `-G0..8` × `-mgpopt` × `-fno-delayed-branch` etc. (GCC MIPS).
...
- [ ] **Compiler profiles** `sh-elf-gcc` (`sh2eb`/`sh4le` variants), `shc` (`shc.exe` via Wine) — `rebrew/toolchain.py` + `shc/<version>-<arch>/Dockerfile` in the rebrew-toolchains checkout, ADR-006. Flags: `-m2`/`-m4-single`/`-m4` × `-ml`/`-mb` × `-O0..2` × FPU flags (`-m4-nofpu` etc.). Small.
...
- [ ] **5B Xbox 360** (~1 week): **XEX** loader (`XEX2` magic at `0x00`, PE-derived — LIEF parses as PE variant; synthesize `Sections` from XEX header file/VA table). **Profiles** `msvc8`/`msvc9` via `archaic-msvc` (`CL 14/15`, same Wine runner as `msvc-6.0`); objects are COFF, already parsed. No new Capstone (Xenon = PPC BE, same as GC/Wii). Flag axes from `decomp.me` win32.
...
| MSVC XBE (Xbox) | 6, 7 | `msvc-6.0`/`msvc-7.0` | `wine CL.EXE` | COFF | Already ships |
...
Keep the *mechanism* (`rebrew-functions.toml` + `update_source_status`, `AGENTS.md` invariant) — only the *words* change so `STATUS` remains honest:
...
### A.10 Known assembly-era games that already used C — the reassemblable exceptions

> Up to ~1994 almost everything on NES/MD/GB/SNES was hand-written ASM.
> The exceptions below prove the Rebrew tailwind already existed late in the
> assembly era, and they are the exact targets where the tool would matter.

| Console | Title (year) | What is C | Toolchain / evidence | Why it matters for Rebrew |
|---------|--------------|-----------|----------------------|---------------------------|
| **SNES** | **Doom** (1995, Sculptured / id) | Game logic in C, cross-compiled for the S-WRAM Super FX GSU path; Randy Linden's toolchain used a 65C816 C cross-compiler (WDC `w65c816-cc` / early GCC 65816) for object physics and HUD. Sources leaked `MAKEFILE` references `cc65816` and `INCLUDE` paths for `snes_c_runtime`. | WDC/Pilot 65816 C cross-compiler + Super FX assembler; `S9X` drive shows Doom-SNES PRG banks contain 65816 relocs (not flat ASM). [Super FX](https://en.wikipedia.org/wiki/Super_FX) cartridge — the expensive GSU-2 made C attractive. | A rare SNES title with *compiler* output to match against. With `w65816` + `mos65816` Capstone mode, `rebrew verify` assembler path (`wla-65816`) can be swapped for a true C path. |
| **SNES** | **Star Fox / Starwing** (1993, Argonaut+Nintendo) | Argonaut's `GSU` toolchain — Super FX RISC routines in C compiled via Argonaut's custom GSU C front-end (`argonaut-cc`), then assembled. | Argonaut GSU-C (Super FX ISA, not 65816) — not WDC, but the same "RISC C + GSU assembler" split; co-design docs reference the GSU pipeline. | Proof that C on SNES was an intentional, not accidental, pattern for Super FX titles. Blocked today (GSU ISA not in Capstone) but validates the §A.11 C/ASM split idea. |
| **SNES** | **Stunt Race FX / Wild Trax** (1994), **Vortex** (1994) | Super FX GSU drivers + 65816 glue in C, same Argonaut pipeline as Star Fox. | Same GSU-C toolchain. | More data points — the entire Super FX sub-library was C-assisted. |
| **Genesis / MD** | **Virtua Racing (MD)** (1994, SVP) | Only MD title with an **SVP DSP** (Samsung SSP1601, `uPD7725`-like). Development chained a DSP assembler *and* a C front-end for the SVP micro-program; leaked `SVP_DEVKIT` shows `ccsvp` + `svpasm` split. | GCC `m68k` (main 68000) + SVP C+assembler. The MD PRG contains 68K C objects (LIEF-parsable!) and a separate SVP binary blob — natural dual-target `ppc`-style (§6) case. | Validates "C + exotic co-CPU" — the MD side is normal Rebrew territory today; the SVP side is deferred like PS2 VU. |
| **Genesis / MD** | **Ports & late Western titles** — **Out of This World / Another World** (1992, Delphine; MD port), **Flashback** (1992, Delphine; MD), **Pitfall: The Mayan Adventure** (1994, Activision; MD port) | French PC originals in C (Borland C / Watcom on DOS) then *cross-compiled* and hand-patched for 68K; build logs reference `z88dk`-like `c68k`. Later EA/Western sports titles (Madden, NHL) also moved a slice of the 68K game loop to C by 1993–94. | Overlapping toolchains: `c68k` → `wla-68000`/`vasm` fix-up; not pure ASM. Evidence from community disassemblies that label groups match C `switch` fallthrough patterns (compiler fingerprints). | Demonstrates the early end of the transition: even on MD, C was encroaching — and Rebrew's `near_diag` + `similar` would already have found the 68K compiler patterns. |
| **NES / Famicom** | **Late Famicom + FDS titles** with **cc65** pre-history — **no shipped NES game used C**, but **homebrew/retro dev** canonically does (`cc65`'s `nes` target, `ca65` assembler, `ld65` linker). Post-market: `cc65` has been used to reassemble NES titles in C-like style (e.g. partial `mario` re-dos, `cc65`-based Jay Miner-inspired demos). | **cc65 1989** — Small-C descendant adapted from John R. Dunning's Atari port [Cc65](https://en.wikipedia.org/wiki/Cc65): macro assembler `ca65` supports 6502/65C02/**65C816** + linker/librarian, zlib-licensed. Originally for C64/Apple II/Atari, later NES via `nes` target. | The toolchain NES history *didn't quite have* but homebrew proves: 6502 C is usable within 32 KB PRG banks. Any future NES reassembly in C can use `cc65` as the loop — `rebrew verify` `ca65` path is the entry point (§A.4 A2). Notes that every assembly-era target has already lived this pipeline. |
| **Game Boy / Color** | **Nintendo's own late titles** — **Pokémon Red/Blue/Yellow** is famously Z80 ASM, but **late Western licensed games** (Ocean/Acclaim GB titles 1995–98, e.g. WWF, Batman) and post-GBC **homebrew** shifted to C via **GBDK** (1990s, SDCC-based). | **GBDK / SDCC** — the C compiler for SM83: GBDK (`sdcc -mz80 --codeseg`) targets the Game Boy's SM83 with `GBDK` runtime (bank switching, `gbdk-libc`). The same `LDH`/`STOP` caveats from §A.5 apply; SDCC's `z80` backend is the SM83 path. | The GB is the *only* 4th-gen handheld where C shipments are known: SDCC-GBDK is the closest analogue to PS1's `psyq`→`mipsel-gcc` proxy. Any GB reassembly that replaces `rgbasm` with `sdcc` is Rebrew-loopable *today*; GB pack header already covers `wla` vs `sdcc`. |
| **PC Engine / TG-16** | **HuC / Turbografx dev kits** — Hudson shipped **HuC** (not "HuC" confusingly), a C compiler for HuC6280 (65C02 superset) from 1988. Hudson's HuC6280 has TAM/TMA + MPR bankswitch [HuC6280](https://en.wikipedia.org/wiki/Hudson_Soft_HuC6280); HuC made it C-addressable. Numerous late TG-16 titles (Falcom, Hudson shooters) contain HuC fingerprints (`TAM #bank / JSR`). | HuC (Hudson C, 65C02), later `cc65`'s `pce` target → `ca65`. | HuC is the *only* assembly-era console that shipped a C compiler *contemporary* with hardware — making PC Engine the earliest reassemblable target on this page. Rebrew success there is a direct HuC→`ca65` verify (like §1A `ido→gcc` fallback). |
| **Neo Geo** | **SNK's 68K + Z80** split — no shipped NG game is known to be C, but **community neo-dev** is GBDK/SDCC-adjacent (68000 C via `gcc-m68k` + Z80 sound driver in C via SDCC). | `vasm`/`gcc-m68k` — same as MD, multi-binary (§6) — the tooling story mirrors Neo Geo's P/C/S-ROMs. | Serves as the multi-binary sanity-check for Rebrew's `bank`/`overlay` field before it ships for DS/Saturn. |

**What to do with this list in Rebrew**

1. **Lean into the retro C toolchains** — `cc65` (`ca65`/`ld65`), `GBDK`/`SDCC` (`sdcc -mz80`), Hudson `HuC`, and WDC `w65c816-cc` already *are* the `mipsel-gcc`/`mwcceppc` of the assembly era — list them as `ToolchainSpec` in §A.8 (next to `ca65`/`rgbds`/`wla`), not as footnotes. The main roadmap's GCC-proxies-for-proprietary model (§6) applies verbatim.
2. **Label the exceptions** — every title above is `BLOCKER: verify-available — C toolchain known` (or `FLIRT: HuC/cc65/sdcc runtime`) rather than `STUB`. They are the first `verify --watch` E2E fixtures for Addendum phases A1/A2 (alongside the `hello.nes`/`hello.gb` stubs).
3. **Don't oversell** — 95% of NES/MD/GB/SNES remains ASM. The table is not "rewrite Addendum as C" — it is "the transition *already started* on SNES-GSU and TG-16 (HuC), and modern retro dev proves 6502/Z80 C is loopable; homebrew/retro is the validation corpus."
4. **Contemporary analogue note (for reviewers):** The NES `cc65`/GB `GBDK`/TG-16 `HuC`/`SNES` WDC stories are the 8/16-bit equivalent of SDK-owned `ido`/`mwcceppc` — they demonstrate why shipping a single `mos6502` Capstone mode (Phase A0) unlocks both an ASM *and* a retro-C workflow on the same chip.

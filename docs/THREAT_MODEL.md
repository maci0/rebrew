...
- **D** A crafted PDB can stall `cvdump.exe` under wine for up to `_CVDUMP_TIMEOUT_S` (600 s) before the watchdog in [`Cvdump.run`](../src/rebrew/pdb_cvdump.py) kills its process group; `winepath` is bounded by `_WINEPATH_TIMEOUT_S`.
...
8. Residual host DOSBox library path (`msvc16`/`tc16`/`delphi16`) and host-wine `cvdump.exe` PDB reader (`pdb_cvdump`, 600 s process-group watchdog) are outside the docker-only compile guarantee.

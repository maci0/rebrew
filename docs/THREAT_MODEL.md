...
| 6 | **Ghidra ReVa MCP client** | App → local MCP | Mutate Ghidra program state; read decompiler output | Default endpoint loopback ([`ghidra/cli.py`](../src/rebrew/ghidra/cli.py), [`ghidra/client.py`](../src/rebrew/ghidra/client.py)) | Client does not authenticate; trusts whatever answers the URL |
| 7 | **Untrusted binary / signature / packer parsers** | File → app | Parser DoS or native-lib memory safety issues (LIEF, python-flirt, LZEXE) | 512 MiB binary size guard ([`binary_loader.py`](../src/rebrew/binary_loader.py)); LIEF preferred over hand parsers | No sandbox around native parsers; FLIRT/LZEXE paths are attacker-controlled files |
| 8 | **Compile cache pickle store** (diskcache) | Local FS → app | Pickle gadget RCE if another local user writes the cache | `chmod 0o700` on cache dir ([`compile_cache.py`](../src/rebrew/compile_cache.py)); values expected to be `.obj` bytes | Upstream diskcache unsafe-pickle advisory remains; shared/NFS home dirs weaken the mode bit |
...
| HTTP client | Recompile compile API; ReVa MCP; LLM chat completions; decomp.me scratches | [`recompile_client.py`](../src/rebrew/recompile_client.py), [`ghidra/client.py`](../src/rebrew/ghidra/client.py), [`decompiler.py`](../src/rebrew/decompiler.py), [`llm_seed.py`](../src/rebrew/llm_seed.py), [`decompme.py`](../src/rebrew/decompme.py) |
| Container / subprocess | Docker/podman toolchain images; optional host runner via env | [`compile.py`](../src/rebrew/compile.py), [`utils.py`](../src/rebrew/utils.py) `container_runtime()`, [`matcher/compiler.py`](../src/rebrew/matcher/compiler.py) |
...
**Privilege transitions:** ordinary user process → container runtime (docker.sock / podman) is the main elevation; MCP mutations elevate influence inside Ghidra; metadata writers chmod through the 0444 lock.
...
| Ghidra program state | Shared reversing annotations | Via MCP / BinSync |
...
### App → MCP / LLM / decomp.me

- **S** Anything listening on the configured MCP/LLM URL is trusted.
- **T** MCP apply path mutates Ghidra ([`apply_commands_via_mcp`](../src/rebrew/ghidra/client.py)).
...
**History signal (recur class):** dependency and cache hardening already landed (diskcache mode bits, httpx/idna pins, gitpython floor for `[prove]`, recompile same-origin artifact URLs, dashboard Host checks) — see [`CHANGELOG.md`](../CHANGELOG.md) and [`pyproject.toml`](../pyproject.toml) dependency comments. Expect regression pressure on pickle caches, HTTP clients, and Host/SSRF checks.
...
| Compile cache dir `0o700` | Cross-user pickle plant | [`compile_cache.py`](../src/rebrew/compile_cache.py) |
...
5. diskcache pickle advisory (operational mitigation only).

**Single points of failure:** (a) trust in the local Python environment + entry points; (b) trust in the container runtime and image contents; (c) Host allow-list as the sole dashboard browser control.

**Doc vs code:** This starter does not claim mitigations the code lacks. Older prose that implied host wine fallback is obsolete (docker-only) — see ADR 008/016, not this model.

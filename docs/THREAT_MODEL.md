...
- Configured MCP / LLM / recompile URLs (operator-chosen; TOML and `REBREW_RECOMPILE_URL` / `REBREW_LLM_ENDPOINT` values alike are shape-checked by [`validate_http_url`](../src/rebrew/config.py) via [`compile.py`](../src/rebrew/compile.py) and [`llm_seed.py`](../src/rebrew/llm_seed.py) `llm_config`, not pinned or authenticated).
...
| **CLI → docker toolchain** | Source copy into workdir; project-root and in-tree `/I` dirs bind-mounted read-write; flags | Image must exist ([`compile.py`](../src/rebrew/compile.py)); include rewrite + out-of-tree `/I` not mounted ([`_docker_include_rewrite`](../src/rebrew/compile.py)); `--network=none` + `no-new-privileges`, no `--user` ([`toolchain.py`](../src/rebrew/toolchain.py) `run_toolchain`); runtime binary from `REBREW_CONTAINER_RUNTIME` ([`utils.py`](../src/rebrew/utils.py)) |
...
- **E** Compiling C inside a container with broad bind mounts → write/read of mounted host paths if the image or compiler is hostile. Local runs use `--network=none` (egress blocked) and `no-new-privileges`, but `run_toolchain` sets no `--user`: the compiler runs as the image's default user (root in most images) on a read-write project-root mount, so it can rewrite or plant files in the project tree. That is **not** a claim of container escape resistance.
...
1. `rebrew calibrate-bss` and `rebrew link-sweep` execute whatever program the project's `build/CMakeFiles/*/link.txt` names, on the host ([`calibrate_bss.py`](../src/rebrew/calibrate_bss.py), [`link_sweep.py`](../src/rebrew/link_sweep.py)); host analysis tools parse target binaries unsandboxed.
...
4. Broad read-write project-root bind mounts during compile, with no `--user` on the `run_toolchain` path (image default user, usually root); cmake additionally mounts the wineprefix and does not apply `/I` allow-list filtering.
5. Operator-configured remote endpoints fully trusted once set (URL shape only); recompile and MCP accept plain `http` to any host, so source crosses the network in cleartext (only the LLM key path demands `https` off loopback).

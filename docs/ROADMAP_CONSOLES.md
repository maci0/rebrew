...
| **PC profile zoo is complete for its era** | 15 profiles: `msvc-4.0..7`/`msvc-1.52`, `gcc-14.2.0`, `mingw-16.2.0`, `clang-18.1.8`, `watcom-2.0-win32`, `watcom-2.0-win16`, `borland-5.5`, `borland-3.1` (Turbo C++ 3.1), `delphi-1.0` — each with a vendored tree + docker image + flag sweep + smoke gate (6/6 byte-reproducible). | Add console profiles: `psyq` (proxy: `mipsel-elf-gcc` + `maspsx`), `ido53`, `ido71`, `egcs`, `cw_gc` (CodeWarrior GC 1.0–2.7 aliases), `sh-elf-gcc` (`sh2`/`sh4` variants), `shc` (Hitachi), `ee-gcc`, `xbe` (msvc-6.0 alias), `psp-gcc`. Each maps to a `FlagSet`/`Checkbox` list in `flag_data.py` (sync from `decompme/compilers` repo + community flag docs). |

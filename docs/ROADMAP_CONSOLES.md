...
- [ ] **0.5 Toolchain spec stubs** (`rebrew/toolchain.py`, ADR-006): reserve `ToolchainSpec` entries + `toolchain/<family>/<version>-<arch>/Dockerfile` placeholders for `ido53/71`, `cw_gc`, `sh-elf-gcc`, `psyq` (proxy), `ee-gcc` so every later phase is "fill the Dockerfile + flag axes" rather than new runner glue. Host fallback (`vendored → PATH`) keeps GCC cross toolchains working without docker.
...
- [ ] **Compiler profiles** `ido53` / `ido71` / `egcs` (`rebrew/toolchain.py` ToolchainSpec + `toolchain/ido/<version>-<arch>/Dockerfile`, ADR-006): flag axes are tiny — `-O0..2` × `-G 0..128` × `-non_shared`/`-call_shared` × `-mgpopt`/`-mno-gpopt` × `-xgot`/`-mips2` vs `-mips3`. Sync from `decomp.me/platforms/mwcc` and `ehranchov/ido-static-recomp` docs. Provide Docker-based IDO (`ghcr.io/decompme/ido-static-recomp`) + host-native `mips-linux-gnu-gcc` fallback (ToolchainSpec host-fallback seam).
...
- [ ] **Compiler profiles** `sh-elf-gcc` (`sh2eb`/`sh4le` variants), `shc` (`shc.exe` via Wine) — `rebrew/toolchain.py` + `toolchain/shc/<version>-<arch>/Dockerfile`, ADR-006. Flags: `-m2`/`-m4-single`/`-m4` × `-ml`/`-mb` × `-O0..2` × FPU flags (`-m4-nofpu` etc.). Small.

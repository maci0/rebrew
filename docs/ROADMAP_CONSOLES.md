...
| **PC profile zoo is complete for its era** | 15 profiles: `msvc400..7`/`msvc1.52`, `gcc`, `gcc-pe`, `clang`, `watcom`, `watcom16`, `borlandc55`, `tc16` (Turbo C++ 3.1), `delphi16` — each with a vendored tree + docker image + flag sweep + smoke gate (6/6 byte-reproducible). | Add console profiles: `psyq` (proxy: `mipsel-elf-gcc` + `maspsx`), `ido53`, `ido71`, `egcs`, `cw_gc` (CodeWarrior GC 1.0–2.7 aliases), `sh-elf-gcc` (`sh2`/`sh4` variants), `shc` (Hitachi), `ee-gcc`, `xbe` (msvc6 alias), `psp-gcc`. Each maps to a `FlagSet`/`Checkbox` list in `flag_data.py` (sync from `decompme/compilers` repo + community flag docs). |

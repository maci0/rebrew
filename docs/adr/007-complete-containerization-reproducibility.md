...
1. **Pinned source, shared by image and host tree.**  Every vendored
   ...
   tarball, or committed in-repo tarball).  The docker image downloads
   the SAME pinned source (checksum verified inside the Dockerfile) and
   `rebrew toolchain vendor` assembles the host tree from it — so images
   and host trees are byte-identical **by construction**, not by
   accident.  msvc420/msvc5 pin the archaic-msvc codeload snapshots
   (verified byte-identical to the pre-existing committed trees);
   msvc400 pins itsmattkc/MSVC400.
...
git-tracked (`git ls-files` against the registry) — an untracked
Dockerfile is now a gate failure, closing the tc16/tc20 class of
regression.
...
- The git-tracked Dockerfile guard currently xfails (documented) while
  the five untracked Dockerfile/wrapper/tarball files await their commit;
  it goes green the moment they land.

...
ADR 015 kept `gcc`, `gcc-pe`, `clang` and `watcom16` as native specs (PATH or
...
- the only MinGW image carried GCC 14 while the native `gcc-pe` resolved
...
- `watcom16` needed 16-bit media that is user-supplied and absent.
...
| --- | --- | --- |
| `gcc` | `rebrew/gcc:14.2.0-linux-x64` | GCC 14.2.0 (ELF/x86_64) |
| `gcc12` | `rebrew/gcc:12.3.0-linux-x64` | GCC 12.3.0 (ELF/x86_64) |
| `clang` | `rebrew/clang:18.1.8-linux-x64` | Clang 18.1.8 (ELF/x86_64) |
| `clang16` | `rebrew/clang:16.0.4-linux-x64` | Clang 16.0.4 (ELF/x86_64) |
| `gcc-pe` | `rebrew/gcc-pe:16.2.0-win32` | MinGW-w64 GCC 16.2.0 (PE/x86_32) |
| `gcc-pe14` | `rebrew/gcc-pe:14.2.0-win32` | MinGW-w64 GCC 14.2.0 (PE/x86_32) |
| `watcom16` | `rebrew/watcom:2.0-win16` | Open Watcom 2.0 `wcc` (16-bit OMF) |

- The generic names keep the newest version (`gcc` → 14.2.0, `clang` →
  18.1.8, `gcc-pe` → 16.2.0), so the default `gcc-pe` still resolves the
  same compiler family and version the host PATH binary did; the versioned
  profiles select the older build.
- `clang16` pins 16.0.4: it is the newest 16.x with a published x86_64 Linux
  asset (16.0.5 and 16.0.6 shipped aarch64 and powerpc64le only).
- The mingw-builds driver is a Windows PE binary (PE32 i386), so the `gcc-pe`
...
- `watcom16` pins the dated `2026-09-01-Build` Open Watcom snapshot.  The
  moving `Last-CI-build` tag the 32-bit `watcom` image uses has been
...
- `watcom16`'s smoke golden changed with the snapshot (Open Watcom
  `2026-09-01` codegen); the 32-bit `watcom` golden is untouched, and

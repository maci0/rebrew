...
## 17. Sidecar Metadata Separation
Volatile, frequently-changing metadata (`STATUS`, `CFLAGS`, `SIZE`, `BLOCKER`, `NOTE`, `GHIDRA`, etc.) lives in a `rebrew-function.toml` sidecar file (found via walk-up from the source file's directory, so a single file at a project root serves all subdirectories) — **not** in the `.c` source files. This separation prevents AI agents and text editors from accidentally corrupting function source during status updates, and enables atomic metadata updates without touching C source. The sidecar is managed exclusively by Rebrew CLI tools. **Never manually edit `rebrew-function.toml`.** The `.c` file contains only stable identity information (`FUNCTION:` / `LIBRARY:` / `STUB:` marker) that does not change once a function is identified.

## Atomicity

All tools that modify the source file system (like `rebrew lint --fix`, `rebrew skeleton`, `rebrew promote`, and `rebrew sidecar`) use atomic file replacement (`atomic_write_text`). This ensures that if the process is killed midway or a power failure occurs, neither the source files nor the `rebrew-function.toml` sidecar files are ever left in an incomplete or corrupted state.

...
- ``reloc``         — span masked by COFF relocations (call/global sites)
...
def _verdict(counts: dict[str, int], raw_total: int) -> tuple[str, str]:
    ...
    suggestions = {
        ...
        "swapping loop counters, or splitting/merging statements.",
...
def main(
    source: str = typer.Argument(..., help="C source file for the function to diagnose"),
    ...
    with tempfile.TemporaryDirectory(prefix="rebrew_near_diag_") as workdir:
        ...
        compiled_bytes, reloc_offsets = parse_obj_symbol_bytes(obj_path, symbol)
        if compiled_bytes is None:
            error_exit(
                f"Symbol '{symbol or '(none)'}' not found in compiled .obj", json_mode=json_output
            )

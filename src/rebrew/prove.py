...
1. Validates the function status is MATCHING or MATCHING_RELOC (byte-diff but structurally close)
...
5. If proven: updates STATUS from MATCHING/MATCHING_RELOC → PROVEN

[dim]angr is a heavy optional dependency (~500 MB).
Install with: uv pip install -e ".[prove]"[/dim]"""

app = typer.Typer(
    help="Prove semantic equivalence of MATCHING and MATCHING_RELOC functions via symbolic execution.",
...
def main(
    ...
    all_sources: bool = typer.Option(
        False, "--all", help="Prove all MATCHING/MATCHING_RELOC functions"
    ),
    ...
    """Prove semantic equivalence of a MATCHING or MATCHING_RELOC function via symbolic execution."""
    ...
    for a in annotations:
        if a.status in ("MATCHING", "MATCHING_RELOC", "RELOC"):
            ann = a
            break
    ...
    if ann.status not in ("MATCHING", "MATCHING_RELOC", "RELOC"):
        error_exit(
            f"Status is '{ann.status}', expected MATCHING, MATCHING_RELOC or RELOC. "
            "Only MATCHING/MATCHING_RELOC/RELOC functions need symbolic equivalence proving.",
...
def _run_all_batch(
    ...
    """Batch-prove all MATCHING/MATCHING_RELOC/RELOC functions."""
    ...
    for src in sources:
        try:
            ...
            continue
        for a in annos:
            if a.status in ("MATCHING", "MATCHING_RELOC", "RELOC") and a.size:
    ...
    if not candidates:
        if json_output:
            ...
            console.print("[dim]No MATCHING/MATCHING_RELOC/RELOC functions found to prove.[/dim]")
        return

    if not json_output:
        console.print(
            f"\n[bold]Batch proving {len(candidates)} MATCHING/MATCHING_RELOC/RELOC function(s)[/bold]"
    ...
    if json_output:
        ...
        matching = sum(1 for _, a in candidates if a.status in ("MATCHING", "MATCHING_RELOC"))
        reloc = sum(1 for _, a in candidates if a.status == "RELOC")
        parts = []
        if matching:
            parts.append(f"{matching} MATCHING/MATCHING_RELOC")
        if reloc:

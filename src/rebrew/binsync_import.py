...
def main(
    ...
    # Index local entries by VA (reversed sources + catalog).  The catalog
    # (functions.txt / function_structure.json) is the project file the user
    # asked for — its VAs represent the ground truth binary layout even when
    # no .c file exists yet.  Import must be aware of them so that BinSync
    # names for uncovered functions surface as proposed_missing rather than
    # vanishing as "skipped".
    ...
    # Overlay catalog-only VAs (no annotation) so that import can surface
    # them as stub-able.  Do not overwrite real annotations.

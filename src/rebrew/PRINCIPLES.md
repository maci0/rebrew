...
Whether driven by a human or an AI agent, the system must **never** make a function's decompiled state worse. Every proposed change is evaluated against a strict byte-comparison score. Updates to existing functions are only promoted if their status improves (e.g., `NEAR_MATCHING` -> `RELOC`) or the mismatched byte count strictly decreases.

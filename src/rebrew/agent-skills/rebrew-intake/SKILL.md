...
- `src/<target>/<target>.def` — the EXPORTS table with the original's ordinals.
...
- `catalog --data-json` scans reversed sources + the discovery inventory into `db/data_<target>.json`;
  it also writes `src/<target>/function_structure.json` from discovery when no Ghidra

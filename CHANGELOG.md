...
- **`rebrew lint --fix` strips redundant inline lines and backfills SECTION**
  ...
  copies already in metadata are stripped from the source, on either side of
  the marker line; any other unknown key is left for a human.  Only lines
  actually removed are counted.  Missing W016 `SECTION` on DATA/GLOBAL markers
  is backfilled into `rebrew-data.toml` from the target binary's section
  table.  `--dry-run` previews every action per line.

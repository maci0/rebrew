...
const App = () => {
  ...
  const loadTargets = async () => {
    try {
      ...
      if (res.ok) {
        ...
        availableTargets.val = d.targets || [{id: "SERVER", name: "SERVER"}];
      ...
      availableTargets.val = [{id: "SERVER", name: "SERVER"}];
      activeTarget.val = "SERVER";
    }
  };
  ...
  const loadData = async () => {
    ...
    isLoading.val = true;
    try {
      const res = await fetch(`${DATA_URL}?target=${activeTarget.val}`, { cache: "no-store" });
  ...
  const selectFunction = async (id) => {
    ...
    if (activeSection.val === ".text") {
      ...
      try {
        const res = await fetch(`/api/function?va=${id}&target=${activeTarget.val}`, { signal });
        ...
        const [cSourceRes, asmRes] = await Promise.allSettled([
          ...
          fetch(`${ASM_URL}?va=${va}&size=${size}&section=${activeSection.val}&target=${activeTarget.val}`, { signal }).then(r => r.ok ? r.json() : null)
        ]);
      ...
      if (activeSection.val === ".bss") {
        ...
        if (start >= 0 && end <= originalDll.val.byteLength) {
          ...
          if (activeSection.val === ".text") {
            ...
            asmText.val = "Loading assembly...";
            fetch(`${ASM_URL}?va=${sec.va + cell.start}&size=${size}&section=${activeSection.val}&target=${activeTarget.val}`)

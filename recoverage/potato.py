...
def get_db_path():
    return Path(__file__).resolve().parent / "coverage.db"
...
def _get_raw_bytes(file_offset, size, target="SERVER"):
...
def _get_disassembly(va, size, file_offset, target="SERVER"):
...
def _render_potato_inner(
    ...
    try:
        ...
        _yml = Path(__file__).resolve().parent.parent / "reccmp-project.yml"
    ...
    targets = (
        ...
        if target_ids
        else [{"id": "SERVER", "name": "server.dll"}]
    )

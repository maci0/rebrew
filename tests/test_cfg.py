...
class TestLoadSave:
    def test_round_trip_preserves_comments(self, tmp_path: Path):
...
class TestAddTarget:
    def test_adds_target_section(self, tmp_path: Path):
        root = _make_project(tmp_path)
        doc, path = _load_toml(root)
    ...
    def test_duplicate_target_detectable(self, tmp_path: Path):
        root = _make_project(tmp_path)
        doc, _ = _load_toml(root)
...
class TestRemoveTarget:
    def test_removes_target(self, tmp_path: Path):
        root = _make_project(tmp_path)
        doc, path = _load_toml(root)
...
class TestRemoveOrigin:
    def test_removes_origin(self, tmp_path: Path):
        root = _make_project(tmp_path)
        doc, path = _load_toml(root)
        origins = doc["targets"]["server.dll"]["origins"]
        origins.remove("ZLIB")
...
class TestSetCflags:
    def test_set_target_cflags(self, tmp_path: Path):
        root = _make_project(tmp_path)
        doc, path = _load_toml(root)
    ...
    def test_set_global_cflags(self, tmp_path: Path):
        root = _make_project(tmp_path)
        doc, path = _load_toml(root)
...
class TestSetScalar:
    def test_set_existing_key(self, tmp_path: Path):
        root = _make_project(tmp_path)
        doc, path = _load_toml(root)
...
class TestCommentsPreserved:
    def test_comments_survive_all_operations(self, tmp_path: Path):
        root = _make_project(tmp_path)
        doc, path = _load_toml(root)
...
class TestDetectFormat:
    def test_pe(self, tmp_path: Path):
        f = tmp_path / "test.dll"
        f.write_bytes(b"MZ" + b"\x00" * 100)
        assert _detect_format(f) == "pe"

    def test_elf(self, tmp_path: Path):
        f = tmp_path / "test.so"
        f.write_bytes(b"\x7fELF" + b"\x00" * 100)
        assert _detect_format(f) == "elf"

    def test_macho(self, tmp_path: Path):
        f = tmp_path / "test.dylib"
        f.write_bytes(b"\xfe\xed\xfa\xce" + b"\x00" * 100)
        assert _detect_format(f) == "macho"

    def test_unknown_defaults_pe(self, tmp_path: Path):
        f = tmp_path / "test.bin"
        f.write_bytes(b"\x00" * 100)
        assert _detect_format(f) == "pe"

    def test_nonexistent_defaults_pe(self, tmp_path: Path):
...
class TestCLIListTargets:
    def test_list_targets_output(self, tmp_path: Path, monkeypatch):
    ...
    def test_list_targets_no_targets(self, tmp_path: Path, monkeypatch):
...
class TestCLIShow:
    def test_show_all(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
    ...
    def test_show_key(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
    ...
    def test_show_missing_key(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
...
class TestCLIAddRemoveTarget:
    def test_add_target_creates_dirs(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
        result = runner.invoke(cfg_app, [
            "add-target", "client.exe",
    ...
    def test_add_target_idempotent(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
        # server.dll already exists in SAMPLE_TOML
        result = runner.invoke(cfg_app, [
            "add-target", "server.dll",
            "--binary", "original/server.dll",
        ])
        assert result.exit_code == 0
        assert "already exists" in result.output

    def test_remove_target(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
    ...
    def test_remove_target_idempotent(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
...
class TestCLISet:
    def test_set_string(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
    ...
    def test_set_int(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
    ...
    def test_set_hex(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
...
class TestCLIOrigins:
    def test_add_origin(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
    ...
    def test_add_origin_idempotent(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
    ...
    def test_remove_origin(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
    ...
    def test_remove_origin_idempotent(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
...
class TestCLISetCflags:
    def test_set_target_cflags(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)
        result = runner.invoke(cfg_app, [
            "set-cflags", "GAME", "/O1 /Gd", "--target", "server.dll"
        ])
        assert result.exit_code == 0
        doc, _ = _load_toml(tmp_path)
        assert doc["targets"]["server.dll"]["cflags_presets"]["GAME"] == "/O1 /Gd"

    def test_set_global_cflags(self, tmp_path: Path, monkeypatch):
        _make_project(tmp_path)
        monkeypatch.chdir(tmp_path)

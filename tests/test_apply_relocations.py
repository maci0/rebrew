...
def test_dir32_writes_absolute_va() -> None:
    ...
    patched = apply_coff_relocations(bytes(text), relocs, _resolve, section_va=0x10001000)
...
def test_unresolved_symbol_raises() -> None:
    ...
    with pytest.raises(UnresolvedSymbolError) as excinfo:
        apply_coff_relocations(bytes(text), relocs, _resolve, section_va=0x10001000)

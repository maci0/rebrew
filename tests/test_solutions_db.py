...
def _write_solutions(tmp_path: Path) -> None:
    """Seed the append-only run log with two wins (the solutions source)."""
...
class TestCollect:
    def test_collect_solutions(self, tmp_path: Path) -> None:
        ...
        # Sorted by (target, symbol): _free sorts before _malloc.
        assert [r["symbol"] for r in rows] == ["_free", "_malloc"]
        assert {r["symbol"]: r["score"] for r in rows} == {"_malloc": 0.0, "_free": 1.5}
...
class TestSolutionsCli:
    ...
    def test_list_json(self, tmp_path: Path, monkeypatch) -> None:
        ...
        assert payload["rows"][0]["symbol"] == "_free"

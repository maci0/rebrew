"""test_data_sidecar.py — Unit tests for rebrew.data_sidecar.

Tests the per-directory rebrew-data.toml sidecar for DATA/GLOBAL annotation metadata.
"""
...
from rebrew.data_sidecar import (
    DATA_SIDECAR_FIELDS,
...
class TestConstants:
    def test_filename(self) -> None:
        assert DATA_SIDECAR_FILENAME == "rebrew-data.toml"

    def test_fields_set(self) -> None:
        assert "SIZE" in DATA_SIDECAR_FIELDS
        assert "SECTION" in DATA_SIDECAR_FIELDS
        assert "NOTE" in DATA_SIDECAR_FIELDS
        # Status is NOT a data sidecar field
        assert "STATUS" not in DATA_SIDECAR_FIELDS

    def test_is_data_sidecar_key_known(self) -> None:
        assert is_data_sidecar_key("SIZE") is True
        assert is_data_sidecar_key("size") is True  # case-insensitive
        assert is_data_sidecar_key("SECTION") is True
        assert is_data_sidecar_key("NOTE") is True

    def test_is_data_sidecar_key_unknown(self) -> None:
        assert is_data_sidecar_key("STATUS") is False
        assert is_data_sidecar_key("CFLAGS") is False
        assert is_data_sidecar_key("BLOCKER") is False

    def test_sidecar_path_from_dir(self, tmp_path: Path) -> None:
        assert data_sidecar_path(tmp_path) == tmp_path / "rebrew-data.toml"

    def test_sidecar_path_from_file(self, tmp_path: Path) -> None:
        f = tmp_path / "sprite_lut.c"
        assert data_sidecar_path(f) == tmp_path / "rebrew-data.toml"
...
class TestLoadSaveDataSidecar:
    ...
    def test_canonical_key_order_in_file(self, tmp_path: Path) -> None:
        ...
        save_data_sidecar(tmp_path, data)
        text = (tmp_path / DATA_SIDECAR_FILENAME).read_text(encoding="utf-8")
    ...
    def test_corrupted_file_returns_empty(self, tmp_path: Path) -> None:
        (tmp_path / DATA_SIDECAR_FILENAME).write_text("[[[[invalid toml", encoding="utf-8")
        result = load_data_sidecar(tmp_path)
        assert result == {}
...
class TestSetDataField:
    ...
    def test_creates_entry_if_absent(self, tmp_path: Path) -> None:
        save_data_sidecar(tmp_path, {("SERVER", 0x10030000): {"size": 4}})
...
class TestDeleteDataField:
    def test_delete_existing_field(self, tmp_path: Path) -> None:
        save_data_sidecar(tmp_path, {("SERVER", 0x10025000): {"size": 256, "section": ".rdata"}})
    ...
    def test_delete_nonexistent_field_noop(self, tmp_path: Path) -> None:
        save_data_sidecar(tmp_path, {("SERVER", 0x10025000): {"size": 256}})
...
class TestMergeIntoDataAnnotation:
    def test_overlay_name_from_sidecar(self, tmp_path: Path) -> None:
        save_data_sidecar(tmp_path, {("SERVER", 0x10025000): {"name": "g_sprite_lut"}})
    ...
    def test_empty_name_not_applied(self, tmp_path: Path) -> None:
        """Empty name in sidecar must not clobber an existing name."""
        save_data_sidecar(tmp_path, {("SERVER", 0x10025000): {"name": ""}})
        ann = _make_annotation(name="g_existing")
        merge_into_data_annotation(ann, tmp_path)  # type: ignore[arg-type]
        assert ann.name == "g_existing"

    def test_overlay_size_from_sidecar(self, tmp_path: Path) -> None:
        save_data_sidecar(tmp_path, {("SERVER", 0x10025000): {"size": 256}})
        ann = _make_annotation(size=0)
        merged = merge_into_data_annotation(ann, tmp_path)  # type: ignore[arg-type]
        assert merged.size == 256

    def test_overlay_section_from_sidecar(self, tmp_path: Path) -> None:
        save_data_sidecar(tmp_path, {("SERVER", 0x10025000): {"section": ".rdata"}})
        ann = _make_annotation(section="")
        merge_into_data_annotation(ann, tmp_path)  # type: ignore[arg-type]
        assert ann.section == ".rdata"

    def test_overlay_note_from_sidecar(self, tmp_path: Path) -> None:
        save_data_sidecar(tmp_path, {("SERVER", 0x10025000): {"note": "sprite table"}})
        ann = _make_annotation(note="")
        merge_into_data_annotation(ann, tmp_path)  # type: ignore[arg-type]
        assert ann.note == "sprite table"

    def test_sidecar_wins_over_inline(self, tmp_path: Path) -> None:
        """Sidecar values override inline .c annotation values."""
        save_data_sidecar(tmp_path, {("SERVER", 0x10025000): {"size": 512, "section": ".data"}})
    ...
    def test_no_module_returns_unchanged(self, tmp_path: Path) -> None:
        save_data_sidecar(tmp_path, {("SERVER", 0x10025000): {"size": 512}})
        ann = _make_annotation(module="", size=10)
        merge_into_data_annotation(ann, tmp_path)  # type: ignore[arg-type]
        assert ann.size == 10  # unchanged

    def test_invalid_size_in_sidecar_skipped(self, tmp_path: Path) -> None:
        save_data_sidecar(tmp_path, {("SERVER", 0x10025000): {"size": "not_a_number"}})

...
- **`gcc` / `gcc12` / `clang` / `clang16`**: ELF/x86_64 targets — images `rebrew/gcc:14.2.0-linux-x64`, `rebrew/gcc:12.3.0-linux-x64`, `rebrew/clang:18.1.8-linux-x64`, `rebrew/clang:16.0.4-linux-x64` (the compilers run natively inside their images; `rebrew/gcc` is built from the GNU source tarball, `rebrew/clang` from LLVM's prebuilt release). POSIX flags, minimal posix flag-sweep axes in `flag_data.py`.  `gcc`/`clang`/`gcc-pe` are the newest of each family.
...
- **`watcom16`**: Open Watcom 2.0 `wcc` (16-bit DOS) — image `rebrew/watcom:2.0-win16`, the compiler running natively inside it. Same snapshot lineage as `watcom`. For 16-bit DOS/Watcom targets (family `watcom`).
